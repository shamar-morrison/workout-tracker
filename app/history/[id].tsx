			...
			>
				<View style={styles.container}>
					<View style={styles.topMeta}>
						<ThemedText style={styles.dateText}>{date.toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' })}</ThemedText>
						<ThemedText style={styles.metaText}>Started at {startTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} • Duration {formatDuration(workout.durationSec)}</ThemedText>
					</View>

					<View style={styles.metricsRow}>
						<View style={styles.metric}><Ionicons name="barbell-outline" size={18} color={colors.text} /><Text style={[styles.metricText, { color: colors.text }]}>Total volume: {workout.totalVolume}</Text></View>
						<View style={styles.metric}><Ionicons name="trophy-outline" size={18} color={colors.text} /><Text style={[styles.metricText, { color: colors.text }]}>{workout.prs} PRs</Text></View>
					</View>
								...
								{ex.sets && ex.sets.length > 0 ? (
									<View style={{ marginTop: 6 }}>
										<View style={styles.setHeaderRow}>
											...
											<Text style={[styles.setHeader, { width: 90, color: colors.text }]}>WEIGHT</Text>
											<Text style={[styles.setHeader, { width: 60, color: colors.text }]}>REPS</Text>
										</View>
										{ex.sets.map((s, i) => (
											<View key={i} style={styles.setRow}>
												<Text style={[styles.setText, { width: 50, color: colors.text }]}>{i + 1}</Text>
												<Text style={[styles.setText, { width: 90, color: colors.text }]}>{s.weight}</Text>
												<Text style={[styles.setText, { width: 60, color: colors.text }]}>{s.reps}</Text>
											</View>
										))}
									</View>
								) : (
									<Text style={{ color: colors.icon }}>Best set: {ex.bestSet ? `${ex.bestSet.weight} × ${ex.bestSet.reps}` : '—'}</Text>
								)}
							</View>
						)}
					/>
